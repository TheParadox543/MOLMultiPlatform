package com.paradox543.malankaraorthodoxliturgica.views
...
@Composable
fun SettingsScreen(settingsViewModel: SettingsViewModel = koinViewModel()) { //navController: NavController,
    val selectedLanguage by settingsViewModel.selectedLanguage.collectAsState()
    val selectedTextStyle by settingsViewModel.selectedTextStyle.collectAsState()
...
//                        fontSize = selectedTextStyle.scaleFactor,
                        fontWeight = FontWeight.Bold,
...
//                    )
                    Text(
                        "About the App",
//                        fontSize = selectedTextStyle.fontSize * 1.2f
                    )
                }
            },
            text = {
...
fun FontSizeDropdownMenu(
    ...
    onOptionSelected: (AppTextStyle) -> Unit
) {
    var expanded by remember { mutableStateOf(false) }
    var selectedText by remember { mutableStateOf(selectedTextStyle) }
    ...
    Box {
        OutlinedButton(
            ...
            Text(selectedText.displayName)
        }
        DropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
            AppTextStyle.entries.forEach { appFontSize ->
                DropdownMenuItem(
                    ...
                    enabled = appFontSize != selectedTextStyle
                )
            }
        }
    }
